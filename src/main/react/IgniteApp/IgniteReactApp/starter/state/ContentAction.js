...
export function failedCreateContent(e) {
  if(typeof(e.response) === 'undefined'){
    ...
    showLoginFailed(strings.IgniteReactApp_Content_already_exists);
  } else if(e.response.status === 500){
    showLoginFailed(strings.IgniteReactApp_Content_unexpected_server_error);
  }
...
// json.Content + ' : ' + JSON.stringify(json));
  ...
  if(GLOBAL.JSESSIONID === null){
    let error = new Error(strings.IgniteReactApp_Content_unexpected_server_error)
